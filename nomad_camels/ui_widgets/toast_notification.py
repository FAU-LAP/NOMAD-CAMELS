...
class ToastConfig:
    ...
    duration: int = 5000  # milliseconds (5 seconds)
    position: str = (
...
class ToastMessageBox(QMessageBox):
    ...
    def _find_main_window(self) -> Optional[QWidget]:
        ...
        if self.parent():
            ...
            while current:
                if (
                    hasattr(current, "windowTitle")
                    and "NOMAD CAMELS" in current.windowTitle()
                ):
                    return current
    ...
    def _position_notification(self):
        ...
        if self.main_window and self.main_window.isVisible():
            ...
            positions = {
                ...
                "center": (
                    window_geometry.x()
                    + (window_geometry.width() - notification_size.width()) // 2,
                    window_geometry.y()
                    + (window_geometry.height() - notification_size.height()) // 2,
                ),
            }
    ...
    def _stack_notification(self, toast: ToastMessageBox):
        ...
        if main_window and main_window.isVisible():
            ...
            for existing_toast in self.active_toasts.values():
                if (
                    existing_toast.isVisible()
                    and existing_toast.config.position == toast.config.position
                ):
                    visible_count += 1
            ...
            if toast.config.position == "top-right":
                ...
                pos = (
                    window_geometry.x()
                    + (window_geometry.width() - toast_size.width()) // 2,
                    window_geometry.y()
                    + (window_geometry.height() - toast_size.height()) // 2,
                )
            ...
            if toast.config.position == "top-right":
                ...
                pos = (
                    screen.width() - toast_size.width() - 20,
                ...
                pos = (
                    (screen.width() - toast_size.width()) // 2,
...
class EnhancedErrorHandler:
    ...
    def handle_validation_error(
        ...
        self.toast_manager.show_toast(
            ...
            config=ToastConfig(duration=5000, auto_dismiss=True),
        )
