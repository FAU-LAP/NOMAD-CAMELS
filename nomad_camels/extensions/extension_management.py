...
from PySide6.QtWidgets import QDialog, QTableWidget, QTableWidgetItem, QLineEdit, QGridLayout, QPushButton, QCheckBox, QSplitter, QTextBrowser, QLabel, QWidget, QDialogButtonBox
...
def get_online_extensions():
    ...
    try:
        ...
        WarnPopup(None, 'Could not (completely) read extension_list.txt from repo.\n'
...
class Extension_Manager(QDialog):
    def __init__(self, preferences, parent=None):
        ...
        self.uninstall_button.clicked.connect(lambda: self.install_selected(uninstall=True))
    ...
    def update_config(self):
        ...
        if self.current_extension:
            self.extension_data[self.current_extension] = self.config_widget.get_config()
...
class Info_Widget(QSplitter):
    ...
    def update_texts(self, extension):
        ...
        try:
            ...
            try:
                meta = importlib_metadata.metadata(f'nomad_camels_extension_{extension}')
        ...
        try:
            ...
            try:
                ...
                for p in pkg_resources.working_set:
                    if not p.key.startswith(f'nomad-camels-driver-{extension.replace("_", "-")}'):
                        continue
...
class Install_Thread(QThread):
    ...
    def run(self):
        try:
            ...
            if self.uninstall:
                ret = subprocess.Popen([sys.executable, '-m', 'pip', 'uninstall', '-y', f'nomad-camels-extension-{name}'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=flags)
            else:
                ret = subprocess.Popen([sys.executable, '-m', 'pip', 'install', f'nomad-camels-extension-{name}'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=flags)
