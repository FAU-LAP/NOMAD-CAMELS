...
class Custom_Function_Signal(Signal):
    ...
    def put(self, value, *, timestamp=None, force=False, metadata=None,
        ...
        if self.put_function:
            retry_function(self.put_function, self.retry_on_error, value)
    ...
    def get(self):
        ...
        if self.read_function:
            self._readback = retry_function(self.read_function, self.retry_on_error)
        return super().get()
    ...
    def trigger(self):
        ...
        if self.trigger_function:
            retry_function(self.trigger_function, self.retry_on_error)
...
def retry_function(func, retries:int, *args, **kwargs):
    excs = []
...
class Custom_Function_SignalRO(SignalRO):
    ...
    trigger_function : callable
        Called when the Signal's `trigger` method is called.
    """
    def __init__(self,  name, value=0., timestamp=None, parent=None, labels=None, kind='hinted', tolerance=None, rtolerance=None, metadata=None, cl=None, attr_name='', read_function=None, trigger_function=None, retry_on_error=0):
        super().__init__(name=name, value=value, timestamp=timestamp, parent=parent, labels=labels, kind=kind, tolerance=tolerance, rtolerance=rtolerance, metadata=metadata, cl=cl, attr_name=attr_name)
        self.read_function = read_function
        self.trigger_function = trigger_function
        self.retry_on_error = retry_on_error
